...
class EntrySummaryStructuredResult(BaseModel):
...
def matches_entrysummary(*, bucket: str, module: str, document: Any) -> bool:
    candidates = [
        bucket,
        module,
    ...
    return any(token in {'entrysummary', 'entrysummaries'} for token in normalized)


def build_prompt() -> str:
    return build_entry_summary_prompt(EntrySummaryStructuredResult)
...
def parse_result(payload: dict[str, Any]) -> EntrySummaryStructuredResult:
    normalized = normalize_payload(payload)
    return EntrySummaryStructuredResult.model_validate(normalized)


def to_prisma_data(*, result: EntrySummaryStructuredResult, raw_data: dict[str, Any]) -> dict[str, Any]:
...
async def persist_extraction(*, prisma, document_id: str, result: EntrySummaryStructuredResult, raw_data: dict[str, Any]):
    ...
    for _ in range(20):
        try:
            return await prisma.entrysummaryextraction.upsert(
