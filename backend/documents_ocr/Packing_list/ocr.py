...
class PackingListStructuredResult(BaseModel):
...
def matches_packinglist(*, bucket: str, module: str, document: Any) -> bool:
    candidates = [
        bucket,
        module,
    ...
    return any(token in {'packinglist', 'packinglists'} for token in normalized)


def build_prompt() -> str:
    return build_packing_list_prompt(PackingListStructuredResult)
...
def parse_result(payload: dict[str, Any]) -> PackingListStructuredResult:
    normalized = normalize_payload(payload)
    return PackingListStructuredResult.model_validate(normalized)


def to_prisma_data(*, result: PackingListStructuredResult, raw_data: dict[str, Any]) -> dict[str, Any]:
...
async def persist_extraction(*, prisma, document_id: str, result: PackingListStructuredResult, raw_data: dict[str, Any]):
    ...
    for _ in range(20):
        try:
            return await prisma.packinglistextraction.upsert(
