...
from documents_ocr.US_cargo_release_order.prompt import build_us_cargo_release_prompt


SCALAR_FIELDS = ['customsBroker','brokerContact','brokerAddress','brokerReferenceNumber','importerAndConsignee','importerAndConsigneeAddress','releasePort','entryNumber','portUnlading','statementPrintDate','scac','truckVesselFlight','itNumber','masterBillOfLading','houseBill1And2']
ARRAY_FIELDS = []
...
def matches_uscargoreleaseorder(*, bucket: str, module: str, document: Any) -> bool:
    candidates = [
        bucket,
        module,
    ...
    return any(token in {'uscargoreleaseorder', 'uscargorelease'} for token in normalized)


def build_prompt() -> str:
    return build_us_cargo_release_prompt(UsCargoReleaseStructuredResult)
...
def parse_result(payload: dict[str, Any]) -> UsCargoReleaseStructuredResult:
    normalized = normalize_payload(payload)
    return UsCargoReleaseStructuredResult.model_validate(normalized)


def to_prisma_data(*, result: UsCargoReleaseStructuredResult, raw_data: dict[str, Any]) -> dict[str, Any]:
...
async def persist_extraction(*, prisma, document_id: str, result: UsCargoReleaseStructuredResult, raw_data: dict[str, Any]):
    ...
    for _ in range(20):
        try:
            return await prisma.uscargoreleaseextraction.upsert(
...
__all__ = [
    "UsCargoReleaseStructuredResult",
    "ValidationError",
