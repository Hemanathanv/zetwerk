...
class UsDeliveryOrderStructuredResult(BaseModel):
...
def matches_usdeliveryorder(*, bucket: str, module: str, document: Any) -> bool:
    candidates = [
        bucket,
        module,
    ...
    return any(token in {'usdeliveryorder', 'usdelivery'} for token in normalized)


def build_prompt() -> str:
    return build_us_delivery_order_prompt(UsDeliveryOrderStructuredResult)
...
def parse_result(payload: dict[str, Any]) -> UsDeliveryOrderStructuredResult:
    normalized = normalize_payload(payload)
    return UsDeliveryOrderStructuredResult.model_validate(normalized)


def to_prisma_data(*, result: UsDeliveryOrderStructuredResult, raw_data: dict[str, Any]) -> dict[str, Any]:
...
async def persist_extraction(*, prisma, document_id: str, result: UsDeliveryOrderStructuredResult, raw_data: dict[str, Any]):
    ...
    for _ in range(20):
        try:
            return await prisma.usdeliveryorderextraction.upsert(
...
__all__ = [
    "UsDeliveryOrderStructuredResult",
    "ValidationError",
