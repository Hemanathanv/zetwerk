...
from documents_ocr.US_delivery_order.ocr import build_prompt, parse_result
...
def _prompt_for_input_path() -> Path:
    while True:
        try:
            raw = input("Enter path to PDF / PNG / JPG for US Delivery Order OCR: ").strip().strip('"').strip("'")
