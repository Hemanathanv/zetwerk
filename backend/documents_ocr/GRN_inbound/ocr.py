...
class GrnInboundStructuredResult(BaseModel):
    ...
    destinationMarks: list[dict[str, Any]] = Field(default_factory=list)


def matches_grninbound(*, bucket: str, module: str, document: Any) -> bool:
    candidates = [
        bucket,
        module,
    ...
    return any(token in {'grninbound', 'inbound', 'grn'} for token in normalized)


def build_prompt() -> str:
    return build_grn_inbound_prompt(GrnInboundStructuredResult)
...
def parse_result(payload: dict[str, Any]) -> GrnInboundStructuredResult:
    normalized = normalize_payload(payload)
    return GrnInboundStructuredResult.model_validate(normalized)


def to_prisma_data(*, result: GrnInboundStructuredResult, raw_data: dict[str, Any]) -> dict[str, Any]:
...
async def persist_extraction(*, prisma, document_id: str, result: GrnInboundStructuredResult, raw_data: dict[str, Any]):
    ...
    for _ in range(20):
        try:
            return await prisma.grninboundextraction.upsert(
