...
class WhToCustomerStructuredResult(BaseModel):
    ...
    otherReferences: list[dict[str, Any]] = Field(default_factory=list)


def matches_whtocustomer(*, bucket: str, module: str, document: Any) -> bool:
    candidates = [
        bucket,
        module,
    ...
    return any(token in {'whcustomer', 'whtocustomer', 'warehousecustomer'} for token in normalized)


def build_prompt() -> str:
    return build_wh_to_customer_prompt(WhToCustomerStructuredResult)
...
def parse_result(payload: dict[str, Any]) -> WhToCustomerStructuredResult:
    normalized = normalize_payload(payload)
    return WhToCustomerStructuredResult.model_validate(normalized)


def to_prisma_data(*, result: WhToCustomerStructuredResult, raw_data: dict[str, Any]) -> dict[str, Any]:
...
async def persist_extraction(*, prisma, document_id: str, result: WhToCustomerStructuredResult, raw_data: dict[str, Any]):
    ...
    for _ in range(20):
        try:
            return await prisma.whtocustomerextraction.upsert(
