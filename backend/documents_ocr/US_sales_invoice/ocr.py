...
class UsSalesInvoiceStructuredResult(BaseModel):
...
def matches_ussalesinvoice(*, bucket: str, module: str, document: Any) -> bool:
    candidates = [
        bucket,
        module,
    ...
    return any(token in {'usinvoice', 'ussalesinvoices', 'ussalesinvoice'} for token in normalized)


def build_prompt() -> str:
    return build_us_sales_invoice_prompt(UsSalesInvoiceStructuredResult)
...
def parse_result(payload: dict[str, Any]) -> UsSalesInvoiceStructuredResult:
    normalized = normalize_payload(payload)
    return UsSalesInvoiceStructuredResult.model_validate(normalized)


def to_prisma_data(*, result: UsSalesInvoiceStructuredResult, raw_data: dict[str, Any]) -> dict[str, Any]:
...
async def persist_extraction(*, prisma, document_id: str, result: UsSalesInvoiceStructuredResult, raw_data: dict[str, Any]):
    ...
    for _ in range(20):
        try:
            return await prisma.ussalesinvoiceextraction.upsert(
...
__all__ = [
    "UsSalesInvoiceStructuredResult",
    "ValidationError",
