...
from documents_ocr.US_sales_invoice.ocr import build_prompt, parse_result
...
def _prompt_for_input_path() -> Path:
    while True:
        try:
            raw = input("Enter path to PDF / PNG / JPG for US Sales Invoice OCR: ").strip().strip('"').strip("'")
