...
class EntrySummaryTariffLinesStructuredResult(BaseModel):
...
def matches_entrysummarytarifflines(*, bucket: str, module: str, document: Any) -> bool:
    candidates = [
        bucket,
        module,
    ...
    return any(token in {'entrysummarytarifflines', 'entrysummarytariffline'} for token in normalized)


def build_prompt() -> str:
    return build_entry_summary_tarifflines_prompt(EntrySummaryTariffLinesStructuredResult)
...
def parse_result(payload: dict[str, Any]) -> EntrySummaryTariffLinesStructuredResult:
    normalized = normalize_payload(payload)
    return EntrySummaryTariffLinesStructuredResult.model_validate(normalized)


def to_prisma_data(*, result: EntrySummaryTariffLinesStructuredResult, raw_data: dict[str, Any]) -> dict[str, Any]:
...
async def persist_extraction(*, prisma, document_id: str, result: EntrySummaryTariffLinesStructuredResult, raw_data: dict[str, Any]):
    ...
    for _ in range(20):
        try:
            return await prisma.entrysummarytarifflineextraction.upsert(
...
__all__ = [
    "EntrySummaryTariffLinesStructuredResult",
    "ValidationError",
