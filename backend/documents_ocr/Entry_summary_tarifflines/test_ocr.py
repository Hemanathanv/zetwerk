...
from documents_ocr.Entry_summary_tarifflines.ocr import build_prompt, parse_result
...
def _prompt_for_input_path() -> Path:
    while True:
        try:
            raw = input("Enter path to PDF / PNG / JPG for Entry Summary Tariff Lines OCR: ").strip().strip('"').strip("'")
