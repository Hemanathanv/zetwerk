...
from documents_ocr.Port_to_WH.prompt import build_port_to_wh_prompt


SCALAR_FIELDS = ['invoice', 'invoiceDate', 'dueDate', 'paymentTerms', 'vendor', 'shipmentId', 'orderNumber', 'customerReferenceNumber', 'pickupLocation', 'pickupDate', 'deliveryLocation', 'deliveryDate', 'containerType', 'mbl', 'containerNumber', 'weightLbs', 'subtotal', 'tax1', 'tax2', 'totalCharge', 'storageDays', 'permitWeight']
ARRAY_FIELDS = ['lineItems']
ARRAY_ITEM_FIELDS: dict[str, list[str]] = {'lineItems': ['chargeDescription', 'units', 'unitRate', 'subtotal']}
...
def matches_porttowh(*, bucket: str, module: str, document: Any) -> bool:
    candidates = [
        bucket,
        module,
    ...
    return any(token in {'transporterbill', 'porttowh', 'portwh'} for token in normalized)


def build_prompt() -> str:
    return build_port_to_wh_prompt(PortToWhStructuredResult)
...
def parse_result(payload: dict[str, Any]) -> PortToWhStructuredResult:
    normalized = normalize_payload(payload)
    return PortToWhStructuredResult.model_validate(normalized)


def to_prisma_data(*, result: PortToWhStructuredResult, raw_data: dict[str, Any]) -> dict[str, Any]:
    ...
    for field_name in SCALAR_FIELDS:
        if field_name == "documentType":
            data[field_name] = _coerce_string(payload.get("documentType"))
        else:
...
async def persist_extraction(*, prisma, document_id: str, result: PortToWhStructuredResult, raw_data: dict[str, Any]):
    ...
    for _ in range(20):
        try:
            return await prisma.porttowhextraction.upsert(
