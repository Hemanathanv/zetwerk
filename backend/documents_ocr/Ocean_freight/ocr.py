...
class OceanFreightStructuredResult(BaseModel):
...
def matches_oceanfreight(*, bucket: str, module: str, document: Any) -> bool:
    candidates = [
        bucket,
        module,
    ...
    return any(token in {'oceanfreight', 'oceanfreightinvoice'} for token in normalized)


def build_prompt() -> str:
    return build_ocean_freight_prompt(OceanFreightStructuredResult)
...
def parse_result(payload: dict[str, Any]) -> OceanFreightStructuredResult:
    normalized = normalize_payload(payload)
    return OceanFreightStructuredResult.model_validate(normalized)


def to_prisma_data(*, result: OceanFreightStructuredResult, raw_data: dict[str, Any]) -> dict[str, Any]:
