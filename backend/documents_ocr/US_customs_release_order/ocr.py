...
from documents_ocr.US_customs_release_order.prompt import build_us_customs_release_prompt


SCALAR_FIELDS = ['customsBroker','brokerContact','brokerAddress','importerNumber','importerNameAndAddress','consigneeBuyingParty','bondType','entryType','suretyCode','portOfEntry','entryNumber','bondValue','portOfUnlading','modeOfTransportation','locationOfGoodsFirms','conveyanceNameFtzZoneId','referenceIdCodeNumber','manufacturer','grossWeight','totalUnits','containers','billOfLadingInformation','signatureOfApplicantAndDate']
ARRAY_FIELDS = []
...
def matches_uscustomsreleaseorder(*, bucket: str, module: str, document: Any) -> bool:
    candidates = [
        bucket,
        module,
    ...
    return any(token in {'uscustomsreleaseorder', 'uscustomsrelease'} for token in normalized)


def build_prompt() -> str:
    return build_us_customs_release_prompt(UsCustomsReleaseStructuredResult)
...
def parse_result(payload: dict[str, Any]) -> UsCustomsReleaseStructuredResult:
    normalized = normalize_payload(payload)
    return UsCustomsReleaseStructuredResult.model_validate(normalized)


def to_prisma_data(*, result: UsCustomsReleaseStructuredResult, raw_data: dict[str, Any]) -> dict[str, Any]:
...
async def persist_extraction(*, prisma, document_id: str, result: UsCustomsReleaseStructuredResult, raw_data: dict[str, Any]):
    ...
    for _ in range(20):
        try:
            return await prisma.uscustomsreleaseextraction.upsert(
...
__all__ = [
    "UsCustomsReleaseStructuredResult",
    "ValidationError",
