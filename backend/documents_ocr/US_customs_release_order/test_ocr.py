...
from documents_ocr.US_customs_release_order.ocr import build_prompt, parse_result
...
def _prompt_for_input_path() -> Path:
    while True:
        try:
            raw = input("Enter path to PDF / PNG / JPG for US Customs Release Order OCR: ").strip().strip('"').strip("'")
