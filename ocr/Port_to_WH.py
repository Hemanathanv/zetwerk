...
DOC_TITLE = "Transporter Bill"
SCHEMA_PATH = Path("data/schemas/transporter-bill.json")
PROMPT_TS = Path("src/prompts/port-to-wh-structured-prompt.ts")

FIELD_KEY_MAP = extract_ts_map(PROMPT_TS, "CANONICAL_JSON_KEY_BY_FIELD_NAME")

LINE_ITEM_FIELDS = ("chargeDescription", "units", "unitRate", "subtotal")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output",
        default="data/transporter_bill_ocr_output.xlsx",
    ...
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        df.to_excel(writer, sheet_name="TRANSPORTER BILL", index=False)
