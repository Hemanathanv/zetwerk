/**
 * Multi-sheet workbook for structured Sales Invoices JSON (object sections + lineItems table).
 */
...
/** Keys that are run metadata / model envelope — not written as section sheets here. */
const ENVELOPE_KEYS = new Set([
  "source",
  "documentType",
]);

export function writeStructuredSalesInvoiceWorkbook(
  outPath: string,
  /** Parsed model JSON: source, documentType, section objects, lineItems[], … */
  data: Record<string, unknown>,
  metaRows: { key: string; value: unknown }[],
  /** Preferred sheet order (camelCase section keys); others appended alphabetically. */
  sectionOrder: string[],
): void {
