/**
 * Multi-sheet workbook for Freight Forwarder Bill JSON.
 */
...
function taxSummarySheet(wb: WorkBook, title: string, raw: unknown): void {
  if (raw && typeof raw === "object" && !Array.isArray(raw)) {
    ...
    if (Array.isArray(rows) && rows.length > 0) {
      tableSheet(wb, `${title}_rows`, rows);
      kvSheet(wb, title, { taxSummary: JSON.stringify(raw) } as Record<string, unknown>);
      return;
    }
  }
  kvSheet(wb, title, {
    taxSummary: raw === null || raw === undefined ? null : typeof raw === "object" ? JSON.stringify(raw) : String(raw),
  } as Record<string, unknown>);
}

function containersSheet(wb: WorkBook, title: string, containers: unknown, totalContainers: unknown): void {
  if (Array.isArray(containers) && containers.length > 0) {
    tableSheet(wb, `${title}_table`, containers);
    kvSheet(wb, title, { totalContainers: totalContainers ?? null } as Record<string, unknown>);
    return;
  }
  kvSheet(wb, title, {
    containers: containers ?? null,
    totalContainers: totalContainers ?? null,
  } as Record<string, unknown>);
}

export function writeStructuredFreightForwarderBillWorkbook(
  outPath: string,
  ...
  containersSheet(wb, "containers", invoiceBody.containers, invoiceBody.totalContainers);
...
export function writeFreightForwarderBillWorkbookFromPayload(
  outPath: string,
  ...
  if (!multi || !invoices?.length) {
    writeStructuredFreightForwarderBillWorkbook(outPath, payload, metaRows);
    return;
  }
  ...
  for (let i = 0; i < invoices.length; i++) {
    ...
    containersSheet(wb, `${p}_cont`, inv.containers, inv.totalContainers);
