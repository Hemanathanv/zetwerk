/**
 * Multi-sheet workbook for Entry Summary JSON (nested lineItems[].tariffLines[] → JSON column).
 */
...
function appendTableSheet(
  ...
  const normalized = arr.map((r) => {
    ...
    if (Array.isArray(o.tariffLines)) {
      o.tariffLines_json = JSON.stringify(o.tariffLines);
      delete o.tariffLines;
    }
    return o;
  });
  const ws =
...
export function writeStructuredEntrySummaryWorkbook(
  outPath: string,
