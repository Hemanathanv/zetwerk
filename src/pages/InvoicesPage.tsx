  ...
  return (
    <div className="p-6 space-y-5">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">Sales, freight, CHA, and forwarder invoices across all shipments</p>
      </div>
        ...
        <div className="flex items-center gap-1 flex-wrap">
          {statusFilters.map(f => (
            <button
              ...
              className={`px-3 py-1 rounded-full text-xs font-medium transition-all ${statusFilter === f ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:text-foreground'}`}
            >
              {f}
            </button>
          ))}
        ...
        <div className="flex items-center gap-1 flex-wrap">
          {typeFilters.map(f => (
            <button
              ...
              className={`px-3 py-1 rounded-full text-xs font-medium transition-all ${typeFilter === f ? 'bg-secondary text-secondary-foreground border border-border' : 'text-muted-foreground hover:text-foreground'}`}
            >
              {f}
            </button>
          ))}
          ...
          <tbody>
            {filtered.map(inv => (
              <tr key={inv.id} className="border-b hover:bg-muted/30 transition-colors" style={{ borderColor: 'hsl(var(--border))' }} data-testid={`row-invoice-${inv.id}`}>
                <td className="py-2.5 px-3 font-semibold text-primary flex items-center gap-1.5">
                  <Receipt className="w-3 h-3 text-muted-foreground flex-shrink-0" />
                  {inv.invoiceNo}
                </td>
                <td className="py-2.5 px-3 font-semibold text-xs">{inv.shipmentId}</td>
                ...
                <td className="py-2.5 px-3 text-muted-foreground">{inv.vendor}</td>
                <td className="py-2.5 px-3 text-muted-foreground">{inv.buyer}</td>
                <td className="py-2.5 px-3 font-semibold">{inv.value}</td>
                <td className="py-2.5 px-3 text-muted-foreground">{inv.issueDate}</td>
                <td className="py-2.5 px-3 text-muted-foreground">{inv.dueDate}</td>
                <td className="py-2.5 px-3 text-muted-foreground">{inv.paymentTerms}</td>
